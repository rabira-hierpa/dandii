...
Status: IMPLEMENTED (2026-08-20, commits e1f7caa + db4b218)
...
## Landing sequence — DONE

Both landed on `feature/invite-flow-amharic-stops`: `e1f7caa` (writes) and
`db4b218` (reads). Two corrections the build forced, worth keeping in the
record:

- The count was **12 read surfaces, not 11**. `console/analytics/report`
  was missed by the manual survey and caught by the coverage test on its
  first run — the guard justified itself before it ever guarded a
  regression.
- The first placement of the closure guard sat inside
  `kind !== "WHOLE_ROUTE"`, so closing an entire route skipped the check.
  Caught by a test written in the same commit.
