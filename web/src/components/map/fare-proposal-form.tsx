...
type TierDraft = { label: string; fromKm: number; toKm: number | null; amount: string };
...
export function FareProposalForm({
  ...
  if (!signedIn) {
    return (
      <Link
        href="/sign-in"
        className="mt-2 block w-full rounded-lg border border-[#D6DCD0] bg-white py-2 text-center text-[12.5px] font-semibold text-[#1A73E8] hover:bg-[#F8FBFF]"
      >
  ...
  if (done) {
    return (
      <div className="mt-2 rounded-lg bg-[#DCFCE7] px-3 py-2.5 text-[12.5px] font-medium text-[#166534]">
  ...
  if (!open) {
    return (
      <button
        ...
        className="mt-2 w-full cursor-pointer rounded-lg border border-[#D6DCD0] bg-white py-2 text-[12.5px] font-semibold text-[#3D4A3F] hover:bg-[#F4F5F2]"
      >
        Suggest a fare correction
      </button>
    );
  }
  ...
  const submit = () => {
    ...
    if (kind === "FLAT") {
      ...
      input = { routeId, kind: "FLAT", flatAmountEtb: amount, note: note || undefined };
    } else {
      ...
      if (parsed.some((t) => !Number.isFinite(t.amountEtb) || t.amountEtb <= 0)) {
        setError("Each tier needs an amount greater than 0");
        return;
      }
      input = { routeId, kind: "TIERED", tiers: parsed, note: note || undefined };
    }
  ...
  return (
    <div className="mt-2 flex flex-col gap-2.5 rounded-xl border border-[#D6DCD0] bg-[#F8FAF6] p-3">
